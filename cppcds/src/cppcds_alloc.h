...
// This part just realize the memory allocate and memory pools management.
namespace cppcds
{
	template <int _inst>
	...
	public:
		static void* allocate(size_t n)
		{
			...
			if (result == NULL)				// if fail to allocate then will call the oom_malloca(n)
			{
				result = oom_malloc(n);
			}
		...
		// This function will free the memory and return memory back to sys.
		static void deallocate(void* p)
		{
			free(p);
		}

		// pointer_name = (data_type*)reallocate*(new_size) the new_size must bigger than old one.
		//If not will lost data
	...
	// Unlimited maclloc the memory to the object if the sys have memory to allcate.
	template <int _inst>
	void*
